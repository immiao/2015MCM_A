...
double dis[N+1][N+1];
double c[N+1];//每个重灾区所需的药物量，若是药厂则代表药厂所拥有的药物数量
long long S[N+1],I[N+1],R[N+1];//每个重灾区的SIR
